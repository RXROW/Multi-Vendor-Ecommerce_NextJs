...
export default async function page({ params: { id } }: any) {
  ...
  const { orderItem = [] } = order; // Fallback to an empty array if orderItem is undefined

  // Calculate subtotal with proper validation
  const subTotal = Array.isArray(orderItem)
    ? orderItem
        .reduce((acc, item) => {
                      ...
                      {Array.isArray(orderItem) &&
                        orderItem.map((item, i) => (
                          ...
                          >
                            <div className="flex items-stretch">
                              <div className="flex-shrink-0">
                                <Image
                                  ...
                                  src={item.imageUrl || "/placeholder.png"} // Fallback to placeholder
                            ...
                            <div className="ml-auto">
                              <p className="text-sm font-bold text-right text-gray-900 dark:text-gray-300">
                              <span>Order Number: </span>  ${item.orderNumber}
                              </p>
                            </div>
                          </li>
                        ))}
