...
export default function CustomDataTable() {
  ...
  return (
    <div className="mt-8 z-0 overflow-hidden hidden md:block">
      <h2 className="text-xl font-bold mb-4 px-4 text-slate-800 dark:text-slate-50 ">
        Recent Orders
      </h2>
      <div className="relative shadow-md sm:rounded-lg p-4 overflow-x-auto">
                ...
                <td className="w-4 p-4">
                  <div className="flex items-center">
                    <input
                      ...
                      className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                    />
                    <label htmlFor={`checkbox-table-search-${i}`} className="sr-only">
                      checkbox
                    </label>
                  </div>
                </td>
                <th scope="row" className="px-6 py-3 font-medium text-gray-800 whitespace-nowrap dark:text-gray-300">{item.id}</th>
                <th scope="row" className="px-6 py-3 font-medium text-gray-800 whitespace-nowrap dark:text-gray-300">{item.first_name}</th>
                <td className="px-6 py-3 text-gray-800 dark:text-gray-300">{item.last_name}</td>
                <td className="px-6 py-3 text-gray-800 dark:text-gray-300">{item.email}</td>
                <td className="px-6 py-3 text-gray-800 dark:text-gray-300">{item.gender}</td>
        ...
        <nav className="flex items-center flex-column flex-wrap md:flex-row justify-between p-4" aria-label="Table navigation">
          <span className="text-sm font-normal text-gray-800 dark:text-gray-300 mb-4 md:mb-0 block w-full md:inline md:w-auto">
            Showing <span className="font-semibold text-slate-800 dark:text-slate-50">{itemStartIndex}-{itemEndIndex}</span> of <span className="font-semibold text-slate-800 dark:text-slate-50">{data.length}</span>
          </span>
